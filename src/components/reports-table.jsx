...
export default function ReportsTable({ reports, loading, onRefresh, onDownload }) {
  ...
  const isStatusCompleted = (report) => {
    ...
    return status && status.toLowerCase() === "finalizado" 
  }
          ...
          <TableBody>
            {sortedReports.length > 0 ? (
              sortedReports.map((report, index) => (
                <TableRow key={getPropertyValue(report, "reporteid") || index}>
                   <TableCell>{getPropertyValue(report, "reporteid")}</TableCell>
                    <TableCell>
                      <span
                  ...
                  <TableCell className="flex gap-2">
                    {isStatusCompleted(report) && (
                      <>
                        ...
                        <ButtonDelete
                          reportId={getPropertyValue(report, "reporteid")}
